  ...
  return (
    <div>
      <label className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-foreground/80">
        {field.label}
        {field.hint && <span title={field.hint} className="inline-flex h-4 w-4 cursor-help items-center justify-center rounded-full bg-muted text-[10px] font-bold text-muted-foreground">?</span>}
      </label>
      <input value={raw} onChange={onChange} placeholder="0" className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm font-medium focus:border-primary focus:outline-none focus:ring-4 focus:ring-primary/10" />
    </div>
  );
}
...
function CalculatorPage() {
  ...
  const salvar = async () => {
    ...
    const { error } = await supabase.from("simulations").insert({
      user_id: user.id, title, inputs: inputs as any, results: {
  ...
  return (
    <div className="space-y-6">
      ...
      <Section title="Dados Iniciais do Planejamento">
        <Grid2>{G1.map((f) => <NumInput key={f.key} field={f} raw={raws[f.key] ?? ""} setRaw={(v) => setRaws({ ...raws, [f.key]: v })} />)}</Grid2>
      </Section>

      <div className="grid gap-6 md:grid-cols-2">
        <Section title="Financiamento Bancário">
          <NumInput field={FIN[0]} raw={raws.prazoF ?? ""} setRaw={(v) => setRaws({ ...raws, prazoF: v })} />
          <Grid2>
            <NumInput field={FIN[1]} raw={raws.jFinAnual ?? ""} setRaw={(v) => setRaws({ ...raws, jFinAnual: v })} />
            <NumInput field={FIN[2]} raw={raws.trAnual ?? ""} setRaw={(v) => setRaws({ ...raws, trAnual: v })} />
          </Grid2>
        </Section>
        ...
        <Section title="Estratégia com Consórcio">
          <Grid2>
            <div>
              <NumInput field={CONS[0]} raw={raws.creditoCons ?? ""} setRaw={(v) => setRaws({ ...raws, creditoCons: v })} />
            ...
            <div>
              <NumInput field={CONS[1]} raw={raws.percLanceEmb ?? ""} setRaw={(v) => setRaws({ ...raws, percLanceEmb: v })} />
          ...
          <Grid2>
            <div>
              ...
              <Toggle value={baseLance} onChange={setBaseLance as any} opts={[["credito", "Sobre Crédito"], ["plano", "Sobre Plano"]]} />
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-semibold">Utilização do Crédito Líquido</label>
              <select value={usoCredito} onChange={(e) => setUsoCredito(e.target.value as any)} className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm font-medium focus:border-primary focus:outline-none">
          ...
          <Grid2>
            <NumInput field={CONS[2]} raw={raws.lanceProprio ?? ""} setRaw={(v) => setRaws({ ...raws, lanceProprio: v })} />
            <NumInput field={CONS[3]} raw={raws.tAdm ?? ""} setRaw={(v) => setRaws({ ...raws, tAdm: v })} />
          </Grid2>
          ...
          <Grid2>
            <NumInput field={CONS[4]} raw={raws.prazoC ?? ""} setRaw={(v) => setRaws({ ...raws, prazoC: v })} />
            <NumInput field={CONS[5]} raw={raws.inccAnual ?? ""} setRaw={(v) => setRaws({ ...raws, inccAnual: v })} />
          </Grid2>
          <div className="rounded-xl border border-primary/20 bg-primary/5 p-3">
            <Grid2>
              <NumInput field={CONS[6]} raw={raws.percReducao ?? ""} setRaw={(v) => setRaws({ ...raws, percReducao: v })} />
              <NumInput field={CONS[7]} raw={raws.mesContemplacao ?? ""} setRaw={(v) => setRaws({ ...raws, mesContemplacao: v })} />
            </Grid2>
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-semibold">Destinação do Lance Contemplado</label>
            <Toggle value={amortTipo} onChange={setAmortTipo as any} opts={[["prazo", "Reduzir Prazo"], ["parcela", "Reduzir Parcela"]]} />
          </div>
        </Section>
      </div>
      ...
      <Section title="Premissas Avançadas & Consultoria" accent>
        <div className="grid gap-4 md:grid-cols-2">
          {ADV.map((f) => <NumInput key={f.key} field={f} raw={raws[f.key] ?? ""} setRaw={(v) => setRaws({ ...raws, [f.key]: v })} />)}
        </div>
      </Section>

      <div className="grid gap-3 md:grid-cols-3">
        <button onClick={calcular_} className="rounded-xl bg-primary px-6 py-4 text-sm font-extrabold uppercase tracking-wide text-primary-foreground shadow-elegant hover:opacity-95 md:col-span-1">Calcular Cenários</button>
        <button onClick={salvar} disabled={!results || saving} className="rounded-xl border border-border bg-card px-6 py-4 text-sm font-extrabold uppercase tracking-wide hover:bg-accent disabled:opacity-50">{saving ? "Salvando…" : "Salvar no Histórico"}</button>
        <button onClick={exportPDF} disabled={!results} className="rounded-xl bg-success px-6 py-4 text-sm font-extrabold uppercase tracking-wide text-success-foreground hover:opacity-95 disabled:opacity-50">Exportar PDF</button>
      </div>

      {results && <ResultsView r={results} usoCredito={usoCredito} valorImovel={inputs.valorImovel} entrada={inputs.entrada} credito={inputs.creditoCons} />}
...
function Toggle<T extends string>({ value, onChange, opts }: { value: T; onChange: (v: T) => void; opts: [T, string][] }) {
  return (
    <div className="flex gap-2">
      {opts.map(([v, l]) => (
        <button key={v} onClick={() => onChange(v)} className={`flex-1 rounded-lg border-2 px-3 py-2.5 text-xs font-bold transition ${value === v ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-muted-foreground hover:border-primary/40"}`}>{l}</button>
      ))}
    </div>
  );
}

function ResultsView({ r, usoCredito, valorImovel, entrada, credito }: { r: CalcResults; usoCredito: "comprar" | "patrimonio"; valorImovel: number; entrada: number; credito: number }) {
  return (
    <div className="space-y-6">
      ...
      <Section title="Quadro Analítico de Patrimônio">
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <Analytic title="Cenário Consórcio Estratégico" rows={[
            ...
            [usoCredito === "patrimonio" ? "Imóvel Adquirido" : "Valor do Imóvel (Corrigido)", usoCredito === "patrimonio" ? "Não aplicável" : fmtBRL(r.imovelNoFuturo), usoCredito === "patrimonio" ? "muted" : "success"],
            [usoCredito === "patrimonio" ? "Patrimônio Total (CDI + Carta)" : "Patrimônio Líquido Final (CDI)", fmtBRL(r.patrimonioConsTotal), "success-strong"],
          ]} />
        </div>
      </Section>
    </div>
...
function Analytic({ title, rows }: { title: string; rows: [string, string, string?][] }) {
  const color = (s?: string) =>
    s === "danger" ? "text-danger" : s === "success" ? "text-success font-bold" : s === "success-strong" ? "text-success font-extrabold" :
    s === "info" ? "text-primary font-bold" : s === "muted" ? "text-muted-foreground" : s === "bold" ? "font-bold" : "";
  return (
      ...
      <div className="mt-3 space-y-2">
        {rows.map(([k, v, c], i) => (
          <div key={i} className={`flex justify-between text-sm ${i === rows.length - 1 ? "border-t border-dashed border-border pt-2 font-bold" : ""}`}>
            <span className="text-foreground/70">{k}</span><span className={color(c)}>{v}</span>
          </div>
        ))}
      </div>
    </div>
...
function ChartParcelas({ r }: { r: CalcResults }) {
  ...
  const path = (data: number[]) => data.map((v, i) => `${i === 0 ? "M" : "L"}${(i / (n - 1 || 1)) * w},${h - (v / max) * h}`).join(" ");
  return (
    <Section title="Evolução das Parcelas ao Longo do Tempo">
      <div className="overflow-x-auto rounded-xl border border-border bg-card p-4">
        <svg viewBox={`0 0 ${w} ${h}`} className="h-64 w-full">
          ...
          <path d={path(r.parcelasPrice)} stroke="oklch(0.74 0.16 65)" strokeWidth="2" fill="none" />
          <path d={path(r.parcelasCons)} stroke="oklch(0.55 0.18 268)" strokeWidth="3" fill="none" />
        </svg>
...
function ChartAlavancagem({ r }: { r: CalcResults }) {
  ...
  const path = (d: number[]) => d.map((v, i) => `${i === 0 ? "M" : "L"}${(i / (n - 1 || 1)) * w},${h - (v / max) * h}`).join(" ");
    ...
    <Section title="Evolução Patrimonial vs Custo Global (Consórcio)">
      <div className="overflow-x-auto rounded-xl border border-border bg-card p-4">
        <svg viewBox={`0 0 ${w} ${h}`} className="h-64 w-full">
          ...
          <path d={path(r.desembolsoCons)} stroke="oklch(0.55 0.21 25)" strokeWidth="3" fill="none" />
          <path d={path(r.patrimonioCons)} stroke="oklch(0.62 0.16 152)" strokeWidth="3" fill="none" />
        </svg>
        <div className="mt-3 flex justify-center gap-4 text-xs">
...
function Legend({ color, label }: { color: string; label: string }) {
  return <div className="flex items-center gap-1.5"><span className="inline-block h-2.5 w-4 rounded" style={{ background: color }} />{label}</div>;
}

function PDFReport({ r, usoCredito, inputs }: { r: CalcResults | null; usoCredito: "comprar" | "patrimonio"; inputs: CalcInputs }) {
  if (!r) return null;
  const valor = inputs.valorImovel;
  const cell: React.CSSProperties = { padding: "8px 12px", borderBottom: "1px solid #f1f2f6", fontSize: 12 };
  const th: React.CSSProperties = { background: "#f1f2f6", color: "#1a2a6c", padding: "8px 12px", fontSize: 11, textAlign: "left", textTransform: "uppercase", fontWeight: 800 };
  return (
    <div style={{ padding: "20mm", width: "210mm", fontFamily: "Inter, sans-serif", color: "#2f3640", background: "#fff" }}>
      <div style={{ borderBottom: "3px solid #1a2a6c", paddingBottom: 15, marginBottom: 25, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        <div>
          <div style={{ fontSize: 22, color: "#1a2a6c", fontWeight: 800, textTransform: "uppercase" }}>Quanto custa? Imobiliário</div>
          <div style={{ fontSize: 11, color: "#7f8c8d", fontWeight: 700, textTransform: "uppercase" }}>Relatório de Inteligência Patrimonial</div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 25 }}>
        {[["Custo Global SAC", r.tSAC, "#b21f1f"], ["Custo Global PRICE", r.tPrice, "#f39c12"], ["Custo Global Consórcio", r.tCons, "#1a2a6c"]].map(([t, v, c]) => (
          <div key={t as string} style={{ padding: 12, borderRadius: 8, background: c as string, color: "#fff" }}>
            <div style={{ fontSize: 9, opacity: 0.9, textTransform: "uppercase", fontWeight: 700 }}>{t as string}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 25 }}>
        <div>
          <div style={{ fontSize: 12, color: "#1a2a6c", fontWeight: 800, borderBottom: "1px solid #dcdde1", paddingBottom: 4, marginBottom: 8, textTransform: "uppercase" }}>Estrutura de Financiamento</div>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr><th style={th}>Variável</th><th style={th}>Valor</th></tr></thead>
            <tbody>
              <tr><td style={cell}>Preço do Imóvel</td><td style={cell}>{fmtBRL(valor)}</td></tr>
              <tr><td style={cell}>ITBI/Cartório</td><td style={cell}>{fmtBRL(r.custoItbiFinanciamento)}</td></tr>
              <tr><td style={{ ...cell, color: "#c0392b", fontWeight: 700 }}>Saldo Inicial Banco</td><td style={{ ...cell, color: "#c0392b", fontWeight: 700 }}>{fmtBRL(valor - inputs.entrada)}</td></tr>
              <tr><td style={{ ...cell, color: "#27ae60", fontWeight: 700 }}>Valor Futuro Imóvel</td><td style={{ ...cell, color: "#27ae60", fontWeight: 700 }}>{fmtBRL(r.imovelNoFuturo)}</td></tr>
              <tr><td style={{ ...cell, fontWeight: 700 }}>Patrimônio CDI</td><td style={{ ...cell, fontWeight: 700 }}>{fmtBRL(r.cdiFin)}</td></tr>
            </tbody>
          </table>
        </div>
        <div>
          <div style={{ fontSize: 12, color: "#1a2a6c", fontWeight: 800, borderBottom: "1px solid #dcdde1", paddingBottom: 4, marginBottom: 8, textTransform: "uppercase" }}>Estrutura Alavancada (Consórcio)</div>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr><th style={th}>Variável</th><th style={th}>Valor</th></tr></thead>
            <tbody>
              <tr><td style={cell}>Poder de Compra Líquido</td><td style={cell}>{fmtBRL(inputs.creditoCons - r.valorEmbVisual)}</td></tr>
              <tr><td style={cell}>Lance Embutido</td><td style={cell}>{fmtBRL(r.valorEmbVisual)}</td></tr>
              <tr><td style={{ ...cell, color: "#2980b9", fontWeight: 700 }}>Saldo Pós-Lance</td><td style={{ ...cell, color: "#2980b9", fontWeight: 700 }}>{fmtBRL(r.saldoDevedorNaContemplacao)}</td></tr>
              <tr><td style={{ ...cell, color: "#c0392b" }}>Aluguel (Espera)</td><td style={{ ...cell, color: "#c0392b" }}>{fmtBRL(r.custoAluguelTotal)}</td></tr>
              <tr><td style={{ ...cell, color: usoCredito === "patrimonio" ? "#7f8c8d" : "#27ae60", fontWeight: 700 }}>{usoCredito === "patrimonio" ? "Imóvel Adquirido" : "Valor Futuro Imóvel"}</td><td style={{ ...cell, color: usoCredito === "patrimonio" ? "#7f8c8d" : "#27ae60", fontWeight: 700 }}>{usoCredito === "patrimonio" ? "Não aplicável" : fmtBRL(r.imovelNoFuturo)}</td></tr>
              <tr style={{ background: "#f4f9f4" }}><td style={{ ...cell, color: "#27ae60", fontWeight: 800 }}>{usoCredito === "patrimonio" ? "Patrimônio Total (CDI + Carta)" : "Patrimônio CDI"}</td><td style={{ ...cell, color: "#27ae60", fontWeight: 800 }}>{fmtBRL(r.patrimonioConsTotal)}</td></tr>
            </tbody>
          </table>
        </div>
      </div>
      <div style={{ marginTop: 25, fontSize: 11, color: "#7f8c8d", background: "#f8f9fa", padding: 12, borderRadius: 6 }}>
        <strong>Nota Técnica:</strong> Os custos globais somam aquisição e custo transicional (aluguel). Na estratégia alavancada o poder de compra líquido compõe o patrimônio rendendo juros compostos após a contemplação.
      </div>
    </div>
  );
}
